...
class GeoLocation:
    ...
    def findCategories(self):
        ...
        for doc in self.listDocs:
            scores = doc.scores
            twitter_weight = 1                          # if it's one of twitter's keywords, higher weight
            for word in scores:
                if word in twitter_alert:
                    twitter_weight = 5
                for cat in self.categoriesDict:
                    if word in self.categoriesDict[cat]:
                        if cat in categories:               # category was created
                            ...
                            score += scores[word] * twitter_weight
...
def main():
    ...
    location = GeoLocation(news_tweets)
